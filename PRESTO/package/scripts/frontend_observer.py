...
class FrontendObserver(Script):
    ...
    def install(self, env):
        ...
        if(params.hostname in params.frontend_hosts):
            # 安装observer的节点已经安装了frontend的服务，取消安装
            Logger.info("所有安装Follower节点:{0}".format(params.frontend_hosts))
            raise RuntimeError('Follower与Observer不能安装在同一服务器,请调整配置后重新安装')

        if (params.hostname in params.frontend_follower_hosts):
            # 安装observer的节点已经安装了frontend的服务，取消安装
            Logger.info("所有安装Follower节点:{0}".format(params.frontend_hosts))
            raise RuntimeError('Follower与Observer不能安装在同一服务器,请调整配置后重新安装')
    ...
    def start(self, env):
        ...
        # 将安装包解压到指定目录
        add_be_sql = "\"ALTER SYSTEM ADD OBSERVER \\\"{0}:{1}\\\";\"".format(self.get_ipv4_by_hostname(env,params.hostname),
        ...
        Logger.info("Run add Frontend Observer :" + add_be_sql_all)
        ...
        if result.replace('\n', '').strip() == '0':
            # 执行失败
            Logger.info("添加Frontend Observer成功")
        else:
            Logger.info("Frontend Observer已经添加,无需重复添加")
