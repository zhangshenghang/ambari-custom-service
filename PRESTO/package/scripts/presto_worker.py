...
class Worker(Script):
    ...
    def install(self, env):
        ...
        if(params.hostname in params.coordinator_hosts):
            # 安装worker的节点已经安装了coordinator的服务
            Logger.info("所有安装Coordinator节点:{0}".format(params.frontend_hosts))
            raise RuntimeError('Coordinator与Worker不能安装在同一服务器,请调整配置后重新安装')
