...
doris_base_dir = os.path.join(stack_root, version, 'doris')
elastic_head_base_dir = os.path.join(stack_root, version, 'elasticsearch-head')

doris_fe_conf_base_dir = doris_base_dir + '/fe/conf'
doris_be_conf_base_dir = doris_base_dir + '/be/conf'
...
doris_fe_pid_file = format("{doris_pid_dir}/doris-fe.pid")
doris_be_pid_file = format("{doris_pid_dir}/doris-be.pid")
...
doris_version = "2.0.5"
doris_download = os.path.join('http://', ambari_server_host, 'doris/apache-doris-{0}-bin-x64.tar.gz'.format(doris_version))
doris_noavx2_download = os.path.join('http://', ambari_server_host, 'doris/apache-doris-{0}-bin-x64-noavx2.tar.gz'.format(doris_version))
# Tool for run sql
mysql_tool_file_name='ambari-mysql-tool.jar'
mysql_tool_download = os.path.join('http://', ambari_server_host, 'ambari/tool/'+mysql_tool_file_name)
...
# Download MySQL Tool Link
download_mysql_tool_link = format("rm -f {mysql_tool_file_name}* && wget {mysql_tool_download}")